...
if [ ! -f "${ENV_FILE}" ]; then
  ...
  # Escape '$' as '$$' so Docker Compose does not re-interpolate the password.
  ADMIN_PW_ENV="$(printf '%s' "${ADMIN_INITIAL_PASSWORD}" | sed 's/\$/$$/g')"
  LEGACY_PW_ENV="$(printf '%s' "${ERP1_LEGACY_MSSQL_PASSWORD:-}" | sed 's/\$/$$/g')"
...
ADMIN_INITIAL_PASSWORD=${ADMIN_PW_ENV}
ADMIN_MUST_CHANGE_PASSWORD=${ADMIN_MUST_CHANGE}
APP_VERSION=$(git -C "${ERP1_DIR}" rev-parse --short HEAD 2>/dev/null || echo 0.1.0)

# Legacy Mar-Kov CMS import source (read-only). Supplied at install time via
# ERP1_LEGACY_MSSQL_* env vars; never stored in the (public) git repo.
LEGACY_MSSQL_HOST=${ERP1_LEGACY_MSSQL_HOST:-}
LEGACY_MSSQL_PORT=${ERP1_LEGACY_MSSQL_PORT:-1433}
LEGACY_MSSQL_DB=${ERP1_LEGACY_MSSQL_DB:-CMS}
LEGACY_MSSQL_USER=${ERP1_LEGACY_MSSQL_USER:-sds_readonly}
LEGACY_MSSQL_PASSWORD=${LEGACY_PW_ENV}
EOF
  chmod 600 "${ENV_FILE}"
...
# Read HTTP_PORT/PUBLIC_URL from .env for messaging WITHOUT sourcing it
# (a chosen admin password may contain characters unsafe to `source`).
HTTP_PORT="$(grep -E '^HTTP_PORT=' "${ENV_FILE}" | head -1 | cut -d= -f2-)"
PUBLIC_URL="$(grep -E '^PUBLIC_URL=' "${ENV_FILE}" | head -1 | cut -d= -f2-)"
