...
export class UsersService {
  ...
  async create(dto: CreateUserDto, actor: Actor) {
    ...
    await this.audit.record({
      ...
      changes: [
        { tableName: 'users', recordId: user.id, fieldName: 'email', oldValue: null, newValue: user.email },
        { tableName: 'users', recordId: user.id, fieldName: 'status', oldValue: null, newValue: 'ACTIVE' },
      ],
    });
