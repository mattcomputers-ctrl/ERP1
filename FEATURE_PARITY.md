...
| Purchase Receipts | 🟡 | `ChangeSetReceipt` (9,427 rows) **mirrored + imported** (1:1 with its receipt `ChangeSet`; PK = ChangeSet — the legacy model is one ChangeSet per received line). **Native receiving now live**: `POST /purchase-orders/:id/receive` (program `purchasing.receive`) records a receipt per line — a Context='PO' `ChangeSet` (native id ≥1e9 under the shared id-alloc lock) + a `ChangeSetReceipt` (PSQty = qty received) + bumps `OrdDetail.QtyUsed`; over-receipt allowed, closed POs rejected, IDOR-safe (lines must belong to the PO), atomic hash-chained audit. The Purchasing page's per-PO panel shows ordered / received / backordered per line (received = Σ `ChangeSetReceipt.PSQty`) + the receipt history, with an editable **Receive** form (prefilled to backordered). On-hand Sublot/Lot/Inventory minting deferred (`ChangeSetReceipt.Sublot` is null in this install — receiving records the event + qty). Return-to-supplier / misc receipts pending |
