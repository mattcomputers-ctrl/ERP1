...
| Purchase Receipts | 🟡 | `ChangeSetReceipt` (9,427 rows) now **mirrored + imported** (1:1 with its receipt `ChangeSet`; PK = ChangeSet — the legacy model is one ChangeSet per received line). **Receiving status now surfaced** on the Purchasing page: a per-PO panel showing ordered / received (from `OrdDetail.QtyUsed`) / backordered per line + the full **receipt history** (each Context='PO' ChangeSet + its `ChangeSetReceipt`: date, item, qty, containers). Native *receive* write action + on-hand Sublot/Lot/Inventory minting are the next increment (the latter deferred — `ChangeSetReceipt.Sublot` is null in this install) |
