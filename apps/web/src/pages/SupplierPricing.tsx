...
// Supplier price-version editor: a supplier owns effective-dated PriceVersions of
// per-item PriceDetails (packaging + quantity-break tiers) that PO line-sourcing
// reads. Browsing needs purchasing.priceVersions; every write needs
// purchasing.priceVersionEditor (the API enforces — a 403 surfaces as an error).
...
interface SupplierRow {
  ...
  versions: number; effectiveDate: string | null; effectiveDetails: number;
}
interface ListResp { rows: SupplierRow[]; total: number }
...
export function SupplierPricing() {
  ...
  if (q) params.set('q', q);
  const list = useQuery({ queryKey: ['supplier-pricing', q], queryFn: () => api.get<ListResp>(`/supplier-pricing?${params.toString()}`) });
  const rows = list.data?.rows ?? [];

  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold text-slate-900">Supplier Price Versions</h1>
      <Card>
        <div className="mb-3 max-w-md">
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search suppliers by code…" />
        </div>
        {list.isLoading ? (
          <p className="text-sm text-slate-400">Loading…</p>
        ) : rows.length === 0 ? (
          <p className="text-sm text-slate-400">No suppliers match.</p>
        ) : (
      ...
      {selected != null && <SupplierPanel key={selected} id={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}
...
interface Detail {
  id: number; itemId: number | null; itemCode: string | null; description: string | null; unit: string | null;
  theirCode: string | null; packageType: string | null; perPackageQty: number | null; perPackageUnit: string | null;
  priceByPackage: boolean; manufacturerId: number | null; manufacturer: string | null; tiers: Tier[]; leadTime: number | null;
}
...
function SupplierPanel({ id, onClose }: { id: number; onClose: () => void }) {
  const qc = useQueryClient();
  const detail = useQuery({ queryKey: ['supplier-pricing', id], queryFn: () => api.get<SupplierDetail>(`/supplier-pricing/${id}`) });
  const d = detail.data;
  const invalidate = () => {
    qc.invalidateQueries({ queryKey: ['supplier-pricing', id] });
    qc.invalidateQueries({ queryKey: ['supplier-pricing'] });
  };

  return (
    <Card>
      <div className="mb-3 flex items-start justify-between">
        <h2 className="text-lg font-medium">{d ? d.name : `Supplier #${id}`} <span className="text-sm font-normal text-slate-400">{d?.code}</span></h2>
      ...
      {d && (
        <div className="space-y-6">
          <VersionsSection supplierId={id} versions={d.versions} effectiveVersionId={d.effectiveVersionId} onChange={invalidate} />
          <DetailsSection supplierId={id} effectiveVersionId={d.effectiveVersionId} details={d.details} onChange={invalidate} />
        </div>
      )}
    </Card>
  );
...
function VersionsSection({ supplierId, versions, effectiveVersionId, onChange }: { supplierId: number; versions: Version[]; effectiveVersionId: number | null; onChange: () => void }) {
  const [effectiveDate, setEffectiveDate] = useState(todayIso());
  const [comment, setComment] = useState('');
  const m = useMutation({
    mutationFn: () => api.post(`/supplier-pricing/${supplierId}/versions`, { effectiveDate, comment: comment.trim() || undefined }),
...
type PartyOption = { id: number; code: string; name: string | null };
type TierEntry = { minOrder: string; price: string };

function DetailsSection({ supplierId, effectiveVersionId, details, onChange }: { supplierId: number; effectiveVersionId: number | null; details: Detail[]; onChange: () => void }) {
  const del = useMutation({
    mutationFn: (detailId: number) => api.del(`/supplier-pricing/${supplierId}/details/${detailId}`),
    onSuccess: onChange,
  });

  return (
    <section>
      <div className="mb-1 text-sm font-medium text-slate-700">Prices <span className="font-normal text-slate-400">(effective version)</span></div>
      {effectiveVersionId == null ? (
        ...
        <>
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-slate-200 text-left text-slate-500">
                <tr>
                  ...
                  <th className="py-1 pr-2 font-medium">Mfr</th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {details.map((dt) => (
                  <tr key={dt.id} className="border-b border-slate-100 align-top">
                    ...
                    <td className="py-1 pr-2 text-slate-600">{dt.manufacturer ?? <span className="text-slate-300">—</span>}</td>
                    <td className="py-1 pr-2 text-right">
          ...
          <AddDetailForm supplierId={supplierId} versionId={effectiveVersionId} onChange={onChange} />
        </>
      )}
...
function AddDetailForm({ supplierId, versionId, onChange }: { supplierId: number; versionId: number; onChange: () => void }) {
  ...
  const [mfr, setMfr] = useState<PartyOption | null>(null);
  const [mfrSearch, setMfrSearch] = useState('');

  const items = useQuery({
    queryKey: ['sp-item-options', itemSearch],
    queryFn: () => api.get<{ rows: ItemOption[] }>(`/supplier-pricing/item-options?q=${encodeURIComponent(itemSearch)}`),
    enabled: !item && itemSearch.trim().length >= 1,
  });
  const pkgs = useQuery({
    queryKey: ['sp-pkg-options', pkgSearch],
    queryFn: () => api.get<{ rows: ItemOption[] }>(`/supplier-pricing/item-options?q=${encodeURIComponent(pkgSearch)}`),
    enabled: !pkgType && pkgSearch.trim().length >= 1,
  });
  const mfrs = useQuery({
    queryKey: ['sp-mfr-options', mfrSearch],
    queryFn: () => api.get<{ rows: PartyOption[] }>(`/supplier-pricing/manufacturer-options?q=${encodeURIComponent(mfrSearch)}`),
    enabled: !mfr && mfrSearch.trim().length >= 1,
  });

  const reset = () => {
    setItem(null); setItemSearch(''); setTiers([{ minOrder: '1', price: '' }]);
    setPkgType(null); setPkgSearch(''); setPerPackageQty(''); setPerPackageUnit(''); setPriceByPackage(false);
    setTheirCode(''); setLeadTime(''); setMfr(null); setMfrSearch('');
  };
  ...
  const m = useMutation({
    mutationFn: () => {
      ...
      if (leadTime !== '' && Number.isFinite(Number(leadTime))) body.leadTime = Number(leadTime);
      if (mfr) body.manufacturerId = mfr.id;
      return api.post(`/supplier-pricing/${supplierId}/versions/${versionId}/details`, body);
    },
    ...
    <div className="mt-3 rounded-md border border-slate-200 p-3">
      ...
      {!item && itemSearch.trim().length >= 1 && (
        <div className="mt-1 max-h-40 overflow-y-auto rounded-md border border-slate-200">
          ...
          {!items.isLoading && (items.data?.rows.length ?? 0) === 0 && <div className="px-3 py-2 text-sm text-slate-400">No items match.</div>}
      ...
      {!pkgType && pkgSearch.trim().length >= 1 && (
        <div className="mt-1 max-h-40 overflow-y-auto rounded-md border border-slate-200">
          ...
          {!pkgs.isLoading && (pkgs.data?.rows.length ?? 0) === 0 && <div className="px-3 py-2 text-sm text-slate-400">No package types match.</div>}
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        <Field label="Lead time (days, optional)"><Input type="number" min="0" step="1" value={leadTime} onChange={(e) => setLeadTime(e.target.value)} /></Field>
        <Field label="Manufacturer (optional)">
          {mfr ? (
            <div className="flex items-center gap-2">
              <span className="rounded-md bg-slate-100 px-2 py-1 text-sm font-medium text-slate-700">{mfr.code}</span>
              <button type="button" onClick={() => setMfr(null)} className="text-sm text-slate-500 hover:underline">change</button>
            </div>
          ) : (
            <Input value={mfrSearch} onChange={(e) => setMfrSearch(e.target.value)} placeholder="Pin to a manufacturer…" />
          )}
        </Field>
      </div>
      {!mfr && mfrSearch.trim().length >= 1 && (
        <div className="mt-1 max-h-40 overflow-y-auto rounded-md border border-slate-200">
          {mfrs.isLoading && <div className="px-3 py-2 text-sm text-slate-400">Searching…</div>}
          {!mfrs.isLoading && (mfrs.data?.rows.length ?? 0) === 0 && <div className="px-3 py-2 text-sm text-slate-400">No manufacturers match.</div>}
          {mfrs.data?.rows.map((mm) => (
            <button type="button" key={mm.id} onClick={() => { setMfr(mm); setMfrSearch(''); }} className="flex w-full items-center justify-between px-3 py-1.5 text-left text-sm hover:bg-slate-50">
              <span className="font-medium">{mm.code}</span><span className="text-xs text-slate-400">{mm.name}</span>
            </button>
          ))}
        </div>
      )}
